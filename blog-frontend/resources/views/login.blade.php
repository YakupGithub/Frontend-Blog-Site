    ...
    <section class="bg-gray-200 min-h-screen flex items-center justify-center">
        <div class="bg-gray-300 flex rounded-2xl shadow-lg max-w-3xl p-5 items-center">
          <div class="md:w-1/2 px-8 md:px-16">
            ...
            <p class="text-xs mt-3 text-[#002D74]">Öncelikle giriş yapmalısınız...</p>
            ...
            <form method="POST" action="{{ route('user.login') }}" class="flex flex-col gap-4">
                ...
                @endif
              <input class="p-2 mt-8 rounded-xl border" type="email" name="email" placeholder="E-posta">
                @if ($errors->has('email'))
              ...
              <button style="background-color: rgb(15 23 42);" class="rounded-xl text-white py-2 hover:scale-105 duration-300">Giriş Yap</button>
            </form>


            <div class="mt-5 text-xs border-b border-[#002D74] py-4 text-[#002D74]">
                <a href="/">Giriş yapmadan devam etmek için tıklayın </a>
            </div>

            <div class="mt-3 text-xs flex justify-between items-center text-[#002D74]">
                <p>Hesabınız yok mu?</p>
                <a href="/register">
                    <button class="py-2 px-5 bg-white border rounded-xl hover:scale-110 duration-300">Kaydol</button>
                </a>
            </div>
          </div>
